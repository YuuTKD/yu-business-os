...
# ─── OWNER_APPROVAL_REQUIRED 出力 ─────────────────────────────────────────────
emit_owner() {
  local reason="$1"
  echo ""
  echo -e "${YLW}${BLD}━━━ OWNER_APPROVAL_REQUIRED ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RST}"
  echo -e "${YLW}${BLD}  ゆうさんの承認が必要です${RST}"
  echo -e "${YLW}  ${reason}${RST}"
  echo -e "${YLW}  → 承認なしでは commit / push / Merge を進めません。${RST}"
  echo -e "${YLW}${BLD}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${RST}"
  echo ""
  exit 3
}

# ─── Governance Gate（Phase D-Lite・ローカル diff・gh 非依存・fail-closed）───
# Phase A の Governance Validator をローカル diff に対して先に実行する。
# GO 以外は既存の gh ベース処理へ進む前にここで停止する（fail-closed）。
run_governance_gate() {
  local base="${GOVERNANCE_BASE:-origin/main}"
  local gate="${REPO_ROOT}/scripts/agent/governance_gate.py"
  [[ -f "$gate" ]] || emit_stop "governance_gate.py が見つかりません（fail-closed）。"

  echo -e "${BLD}[0/5] Governance Gate（ローカル判定）${RST}"

  local gate_rc=0
  set +e
  if [[ "${YU_OWNER_APPROVED:-}" == "true" ]]; then
    python3 "$gate" --base "$base" --head HEAD --owner-approved
    gate_rc=$?
  else
  ...
  case "$gate_rc" in
    0)  ok "Governance Gate: GO" ;;
    10) emit_fix "Governance Gate が FIX を返しました。変更内容を見直してください。" ;;
    20) emit_owner "Governance Gate が OWNER_APPROVAL_REQUIRED を返しました。" ;;
    30) emit_stop "Governance Gate が STOP を返しました。危険を解消してください。" ;;
    40) emit_stop "Governance Gate が INTERNAL_ERROR（fail-closed）。判定不能のため停止。" ;;
    *)  emit_stop "Governance Gate が未知の終了コード(${gate_rc})を返しました（fail-closed）。" ;;
  esac
  echo ""
}
...
# ローカル Governance Gate を最初に実行（gh 非依存・fail-closed）。
# GO のときだけ以降の gh ベース Codex レビューへ進む。
run_governance_gate
