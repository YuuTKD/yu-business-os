...
| 対象 | 変更 | 破壊性 |
|---|---|---|
| `CATERING_SALES_TARGETS` | 右端に11列追加 | 低 — 既存コードは `get_all_records()`（ヘッダ→dict）で読むため列追加に耐性あり |
| `02_問い合わせ` | 右端に4列追加（P〜S列） | **中** — `core/catering_report.py:64` が `get_all_values()[2:]` で**位置参照**している。右端追加なら既存インデックスは不変で安全。**左・中間への挿入は禁止** |
| `03_見積` | 右端に1列 | 中（同上、`:71`） |
| `04_受注管理` | 右端に2列 | 中（同上、`:78` — `r[4] == "受注"` の位置参照あり） |
...
- **本番ワークブックの実データ量が不明。** `02_問い合わせ`〜`07_利益管理` にサンプル行だけなのか実データがあるのか、認証情報を使った読み取りをしていないため未確認。**Week1-4 の read-only 棚卸しで最初に確定させる。**
