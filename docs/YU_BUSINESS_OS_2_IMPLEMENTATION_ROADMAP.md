...
### Phase R2 — Change Classification + Test Selection（**実装完了 2026-07-15**）

| 項目 | 内容 |
|---|---|
| 目的 | diff_risk.py 拡張で分類 JSON を出し、必要テストのみ実行 |
| 変更ファイル | `core/governance/diff_risk.py`（`classify_change` 追加・既存関数無変更）/ `scripts/release/classify_change.py`（新規CLI）/ `.github/workflows/pr-validation.yml`（分類+選択実行）/ `tests/governance/test_change_classification.py`（25テスト新規） |
| リスク | HIGH（governance コード変更 → Full suite 強制・本番非接触） |
| テスト | 分類ユニット 25件 + Full suite。**合計 413件 PASS** |
| 完了条件 | 分類 JSON が job summary に出る / run_scope で選択実行される ✅ |
| rollback | merge しない / branch 削除 / revert（既存 `classify_paths` は無変更のため影響なし） |
| ゆうさん判断 | PR Merge YES 1回 |
| 期待削減 | docs-only はテスト0・部分変更は該当グループのみ・重複 Full Test 削減 |

**実装事実**: 分類正本は `diff_risk.py` の**追加関数 `classify_change(paths, registry)`** 1つ
（新規分類器を増やさず既存 `classify_paths`/secret/runaway helper を再利用）。カテゴリ16種
（docs_only / content_policy / sns_post / image_policy / business_config / ssot / core_runtime /
cloud_run_service / scheduler / secret_reference / external_send / acquisition / financial_logic /
deployment_workflow / cross_business / tests＋fail-closed の unknown）。`run_scope` は
**FULL / NONE(docs-only) / GROUPS** の3値で fail-closed（unknown・blocked・空 diff → FULL）。
Full 強制カテゴリ = core_runtime / governance / deployment_workflow / secret_reference /
external_send / cross_business / ssot / cloud_run_service / unknown。affected_businesses・
...
## Phase R2.5 Retention — OWNER_ACCEPTED_EXCEPTION 記録（2026-07-16）

| 項目 | 値 |
|---|---|
| 対象 | `gs://yu-release-ledger` retention policy |
| 実値 | 34495200 秒（≈399日18時間） |
| 目標 | 400日（34560000秒） |
| 差 | 64800秒（18時間） |
| 判定 | **OWNER_ACCEPTED_EXCEPTION**（運用上許容） |
| 承認者 | OWNER（ゆうさん） |
| 承認日 | 2026-07-16 |
| 対応 | Retention Policy 変更なし・Retention Lock なし。`--verify` は `READY_WITH_EXCEPTION` 表示。`ensure_retention` は当該値を SKIP（変更しない） |

R2.5 は本例外込みで **完了扱い**。実インフラ `--verify` = 全 READY（retention のみ READY_WITH_EXCEPTION）。

## Phase R3 — candidate no-traffic + smoke（**実装完了 2026-07-16 / candidate deploy 未実行**）

| 項目 | 内容 |
|---|---|
| 対象 | `trees-catering-ai` のみ（allowlist・他事業は deploy_target=false） |
| 変更ファイル | `.github/workflows/release.yml`（新規・**workflow_dispatch のみ**）/ `scripts/release/smoke_test.py`（新規・read-only）/ `configs/release/services.yaml`（新規・endpoint SSOT）/ `core/entrypoint.py`（`/status` に read-only `release` block 追加）/ `core/governance/diff_risk.py`（configs/release を deployment_workflow に分類）/ `tests/release/test_r3_release.py`（25テスト） |
| フロー | SHA固定Image build→Artifact Registry(yu-release)→`--no-traffic --tag candidate` deploy→candidate URL→read-only smoke。**update-traffic なし**（production traffic 0% 不変） |
| smoke | health200/status200/business identity/image_generation=false/line_text=true/line_image=false/delivery_mode=TEXT_ONLY/secret露出なし。fail-closed(unknown→ROLLBACK) |
| 安全 | manual trigger のみ(merge で自動実行しない) / environment: production(R5承認ゲート) / WIF(id-token) / service allowlist=catering / Scheduler・Secret・env 不変 |
| テスト | 25件 + FULL = **457 PASS** |
| **未実行** | candidate deploy 自体（次の owner YES で workflow_dispatch 実行）。merge もしない |
| 次のYES | 「release.yml を workflow_dispatch で catering に対し実行してよいか」（candidate 作成 + smoke） |
