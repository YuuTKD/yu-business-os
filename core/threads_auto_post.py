...
安全装置:
  ...
  - image_url があれば画像投稿、なければテキスト投稿（フォールバックなし）
...
def run(
    ...
    for p in pending:
        ...
        # 投稿実行（画像あり → 画像投稿、なし → テキスト投稿）
        if p["image_url"]:
            res = publish_image(ss_id, creds_path, valid_key, text, p["image_url"])
        else:
